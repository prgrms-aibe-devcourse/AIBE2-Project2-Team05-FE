...
public class TravelFeedService {
    ...
    public void deleteFeed(Long feedId, Long userId) {
        log.info("🗑️ 피드 삭제 시작: feedId={}, userId={}", feedId, userId);
        ...
        try {
            ...
            if (!feed.getUser().getId().equals(userId)) {
                log.error("❌ 권한 없음: 피드 작성자({})와 요청자({})가 다름", feed.getUser().getId(), userId);
                throw new RuntimeException("피드 삭제 권한이 없습니다.");
            }
            ...
            log.error("❌ 피드 삭제 실패: feedId={}, error={}", feedId, e.getMessage(), e);
            throw new RuntimeException("피드 삭제 실패: " + e.getMessage(), e);
        }
    }
    
    // ✅ travelPlanId로 피드와 여행 계획 함께 삭제 메서드 추가
    public void deleteFeedByTravelPlanId(Long travelPlanId, Long userId) {
        log.info("🗑️ travelPlanId로 피드와 여행 계획 삭제 시작: travelPlanId={}, userId={}", travelPlanId, userId);
        ...
        try {
            ...
            // 여행 계획 가져오기
            TravelPlan travelPlan = feed.getTravelPlan();
            
            // 피드 삭제
            feedRepository.delete(feed);
            log.info("✅ 피드 삭제 완료: feedId={}", feed.getId());
            
            // 여행 계획도 삭제
            if (travelPlan != null) {
                travelPlanRepository.delete(travelPlan);
                log.info("✅ 여행 계획 삭제 완료: travelPlanId={}", travelPlanId);
            }
            
            log.info("✅ 피드와 여행 계획 모두 삭제 완료: travelPlanId={}", travelPlanId);
            
        } catch (Exception e) {
            log.error("❌ 피드/여행 계획 삭제 실패: travelPlanId={}, error={}", travelPlanId, e.getMessage(), e);
            throw new RuntimeException("피드/여행 계획 삭제 실패: " + e.getMessage(), e);
        }
    }
}
